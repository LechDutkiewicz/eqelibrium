...
if ( !function_exists( 'tsm_sm_list' ) ) {
	...
	function tsm_sm_list( $ex_class = null ) {
		...
		if ( have_rows('con_sm', 'options') ) {
			...
			?>
			<ul class="<?= $ex_class; ?> list__social-media">
				<?php
				while ( have_rows( 'con_sm', 'options' ) ) {
					the_row();
					?>
					<li>
						<a href="<?= get_sub_field('con_sm_link'); ?>" class="link__white" title="<?= sprintf(__('Link to our social media profile on %s', THEME_DOMAIN), get_sub_field('con_sm_type')); ?>" target="_blank"><i class="icon-<?= get_sub_field('con_sm_type'); ?> theme_icons-2x"></i></a>
					</li>
					<?php
				}
				?>
...
/*============================================================
=            Get single social media profile link            =
============================================================*/


if ( !function_exists( 'get_sm_profile_link' ) ) {

	function get_sm_profile_link( $profile ) {

		$sm_profiles = get_field('con_sm', 'option');
		$link = false;

		foreach ($sm_profiles as $sm_profile) {
			echo $i;
			if ($sm_profile['con_sm_type'] === $profile) {
				$link = $sm_profile['con_sm_link'];
				$name = $sm_profile['con_sm_name'];
				break;
			} else {
				continue;
			}
		...
		if ( isset($link) ) {
			...
			ob_start();
			?>
			<a href="<?= $link; ?>" title="<?= sprintf(__('Link to our social media profile on %s', THEME_DOMAIN), $profile); ?>" target="_blank"><?= $name; ?></a>
			<?php
...
/*=====  End of Get single social media profile link  ======*/
		...
		'fields'	=> array(
			array(
				...
				'sub_fields'	=> array(
					...
					array(
						'key'	=> 'field_0TfUkZpTZEKPYYb',
						'label'	=> 'Nazwa profilu',
						'description' => 'Podaj tutaj nazwę profilu, która może być widoczna w różnych miejscach strony. Np @Twoja_Firma dla profilu na Facebook.',
						'name'	=> 'con_sm_name',
						'type'	=> 'text'
					),
				)
